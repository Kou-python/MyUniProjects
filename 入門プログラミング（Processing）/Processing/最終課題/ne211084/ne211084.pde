...
void mousePressed() {//マウス
  ...
  if (scene==1) {//戻る
    ...
    /////////////
    if (play==-1) {//ラスト部屋
      if (mouseX>=25&&mouseX<=390&&mouseY>=350&&mouseY<=390) {
        play=0;
      }
      ...
      if (mouseX>=290&&mouseY>=300&&mouseX<=460&&mouseY<=390) {
...
void play() {
  ...
  fill(0);
  triangle(25, 350, 80, 350, 54, 390);
  fill(#A2BEF7);
  rect(212, 160, 168, 114);
  gazo(paper, 130, 300, 0.3);
  gazo(no0, 290, 300, 0.2);
  gazo(no1, 350, 300, 0.2);
  gazo(no2, 410, 300, 0.2);
  gazo(no3, 320, 350, 0.2);
  gazo(no4, 380, 350, 0.2);
